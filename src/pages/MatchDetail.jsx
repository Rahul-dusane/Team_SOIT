...
export default function MatchDetail() {
  ...
  return <div className="mx-auto max-w-7xl"><Link className="mb-6 block text-teal" to="/ranking">← Back to ranking</Link><ErrorBanner message={error} />
    {loading ? <p className="panel p-8">Loading profile and match…</p> : !candidate ? <p className="panel p-8">Profile data unavailable.</p> : <>
      <h1 className="text-3xl font-extrabold">{candidate?.name || 'Unknown Candidate'}</h1><p className="my-3 text-muted">{candidate?.role || 'No role'} · {candidate?.experience || 'No experience data'}</p><p className="mb-6 text-sm">Education: {candidate?.education || 'Not provided'}</p>
      <section className="panel mb-6 p-6"><h2 className="text-xl font-bold">Overall match score</h2><p className="my-3">{match ? `Job: ${job?.title || match?.job_id || 'Unknown Job'}` : 'No saved match analysis available.'}</p><MatchScore score={match?.overall_score} size="lg" />{match && <p className="mt-3">Reported decision: {match?.decision || match?.overall_status || 'Unavailable'}</p>}
      {!match && <div className="mt-5 flex flex-wrap gap-3"><select aria-label="Job to match" value={jobId} onChange={e => setJobId(e.target.value)} className="rounded-lg border p-3"><option value="">Select a saved job</option>{jobs && Array.isArray(jobs) ? jobs.map(j => <option value={j?.job_id || ''} key={j?.job_id || ''}>{j?.title || 'Unknown'}</option>) : null}</select><button className="btn-primary" disabled={!jobId || running} onClick={calculate}>{running ? 'Computing…' : 'Run Match Engine'}</button></div>}</section>
      ...
      <div className="grid gap-6 lg:grid-cols-2"><section className="panel p-6"><h2 className="mb-4 text-xl font-bold">Score breakdown</h2><ScoreBreakdown data={breakdown} /></section><section className="panel p-6"><h2 className="mb-4 text-xl font-bold">Skills recorded in profile</h2><p className="text-sm">{(candidate?.skills && Array.isArray(candidate.skills)) ? candidate.skills.join(', ') || 'No skills recorded.' : 'No skills recorded.'}</p><h3 className="my-4 font-bold">Requirement assessments</h3>{(match?.assessments && Array.isArray(match.assessments)) && match.assessments.length ? match.assessments.map((a,i) => <div key={a?.requirement_id || i} className="border-b py-3 text-sm"><p>{a?.description || a?.requirement_id || 'Unknown requirement'}</p><p>Status: {a?.status ? a.status.replace('_', ' ') : 'Not satisfied'}</p>{a?.earned_score != null && <p>Earned score: {a.earned_score}{a?.max_score != null ? ` / ${a.max_score}` : ''}</p>}</div>) : <p className="text-sm text-muted">No requirement assessments available.</p>}</section></div>
      <section className="panel mt-6 p-6"><h2 className="mb-4 text-xl font-bold">Recruiter summary</h2><p>{summary?.summary_text || 'No recruiter summary available.'}</p><p className="mt-4">Strengths: {(summary?.key_strengths && Array.isArray(summary.key_strengths)) ? summary.key_strengths.join(', ') || 'Not provided' : 'Not provided'}</p><p className="mt-2">Gaps: {(summary?.key_gaps && Array.isArray(summary.key_gaps)) ? summary.key_gaps.join(', ') || 'Not provided' : 'Not provided'}</p><p className="mt-2">Recommendation: {summary?.recommendation || 'Not provided'}</p></section>
      <section className="panel mt-6 p-6"><h2 className="mb-4 text-xl font-bold">Evidence and uncertainty</h2>{(match?.evidence_items && Array.isArray(match.evidence_items)) && match.evidence_items.length ? match.evidence_items.map((item,i) => <button className="btn-soft mr-3" key={i} onClick={() => setEvidence(item)}>View evidence {i+1}</button>) : <p>No evidence passages provided by the server.</p>}{(match?.uncertainty_flags && Array.isArray(match.uncertainty_flags)) && match.uncertainty_flags.length > 0 && <ul className="mt-4 list-inside list-disc">{match.uncertainty_flags.map((flag,i) => <li key={i}>{flag}</li>)}</ul>}</section>
